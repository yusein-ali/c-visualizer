//-------------------------------------------------------------------------
//  Vendored from JSCPP, https://github.com/felixhao28/JSCPP (MIT).
//  Source: pegjs/ast.pegjs @ 2.0.9.
//
//  PLIVET uses this grammar as a syntax oracle only. The JSCPP runtime -
//  its interpreter, its own preprocessor and its `includes/` library - is
//  never constructed; `JscppSyntax.ts` calls the generated parser and reads
//  nothing but the failure position. Execution stays with unicoen.ts.
//
//  Regenerate `ast.generated.js` with `npm run generate:parser` after any
//  edit here. Two rules are changed from upstream, both marked `PLIVET:`.
//-------------------------------------------------------------------------
...
// PLIVET: a bare `;` at file scope is legal C, and PLIVET emits them itself -
// `FunctionPointerTable.apply` blanks `typedef int (*BinOp)(int, int);` down
// to its semicolon before the parser ever sees the line. Upstream requires
// every external declaration to declare something.
ExternalDeclaration
    = Namespace / TypedefDeclaration / FunctionDefinition / Declaration / EmptyDeclaration
    ;

EmptyDeclaration
    = SEMI {
      return addPositionInfo({type:'EmptyDeclaration'});
    }
    ;
...
// PLIVET: C11 `_Atomic(int) n;`. Upstream has neither this nor the qualifier
// form below. PLIVET's own qualifier pass used to erase both before any parser
// saw them, which is exactly the blindness this grammar is now read to avoid.
AtomicTypeSpecifier
    = ATOMIC LPAR a:TypeName RPAR {
      return a;
    }
    ;
...
// PLIVET: upstream reads `CONST` alone, though it defines the VOLATILE and
// RESTRICT tokens below and never references them - its interpreter models
// neither qualifier. A syntax check has no such excuse: `volatile int x;` is
// C, and refusing it would refuse the program.
TypeQualifier
    = a:(CONST
    / VOLATILE
    / RESTRICT
...
ATOMIC    = a:"_Atomic"       !IdChar Spacing {return a;};
