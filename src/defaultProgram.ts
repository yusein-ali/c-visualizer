...
int main(void) {
  ...
  *pointer = 9;
  ...
  BinaryOperation member_operation = calculator.operation;
  int member_result = member_operation(9, 4);
  int member_display = member_operation(8, 3);
  ...
  int matched = scanf("%d", &entered);
  if (matched != 1) {
    entered = -1;
  }

  /* File I/O uses the browser-side virtual file system. */
  FILE *output_file = NULL;
  output_file = fopen("c-visualizer.txt", "w");
  if (output_file != NULL) {
    fputs("written by the construct tour\n", output_file);
    fclose(output_file);
  }

  char file_text[32] = "file unavailable";
  FILE *input_file = NULL;
  input_file = fopen("c-visualizer.txt", "r");
  if (input_file != NULL) {
    fgets(file_text, 32, input_file);
    fclose(input_file);
  }

  /* BREAKPOINT: inspect memory, aggregates, input, and pointer arrows. */
  LOG("tour ready\n");
  LOG("%s: count=%d scaled=%d line=%d\n", STRINGIFY(PLIVET), ITEM_COUNT, SCALED_COUNT, __LINE__);
  printf("flow=%d once=%d larger=%d widened=%d input=%d\n",
         sum, once, larger, (int)widened, entered);
